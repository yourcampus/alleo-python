...
from alleo._auth import SyncTokenCache
from alleo._core import DEFAULT_BASE_URL, _Core
from alleo._pagination import paginate_employees_sync
...
class AlleoClient:
    """Synchronous client for the Alleo Customer API.
    ...
    def _request(
        ...
        *,
        ...
        while True:
            ...
            if auth_required:
                req.headers["Authorization"] = f"Bearer {self._auth.token()}"
            ...
            # 401 refresh-retry once. Does NOT count toward the retry budget.
    ...
    def get_employee(self, company_id: int, employee_id: int) -> EmployeeOut:
        resp = self._request("GET", f"/companies/{company_id}/employees/{employee_id}")
    ...
    def create_employee(
        ...
        resp = self._request("POST", f"/companies/{company_id}/employees", json=payload)
        return EmployeeOut.model_validate(resp.json())
    ...
    def deactivate_employee(self, company_id: int, employee_id: int) -> EmployeeOut:
        resp = self._request("POST", f"/companies/{company_id}/employees/{employee_id}/deactivate")
        return EmployeeOut.model_validate(resp.json())

    def activate_employee(self, company_id: int, employee_id: int) -> EmployeeOut:
        resp = self._request("POST", f"/companies/{company_id}/employees/{employee_id}/activate")
        return EmployeeOut.model_validate(resp.json())

    def list_company_groups(self, company_id: int) -> list[CompanyGroupOut]:
        resp = self._request("GET", f"/companies/{company_id}/groups")
    ...
    def list_employees_iter(
        ...
        *,
        ...
        def fetch_page(skip: int, limit: int) -> EmployeeListResponse:
        ...
        return paginate_employees_sync(fetch_page)
...
def _dump_create(body: EmployeeCreate | Mapping[str, Any]) -> dict[str, Any]:
    ...
    # Validate dicts too so callers get consistent errors.
